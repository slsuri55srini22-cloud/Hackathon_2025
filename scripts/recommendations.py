class IssueRecommendations:
    def __init__(self):
        # All known issues and their detailed recommendations
        self.data = [
            {
...
- Keep BIOS updated."""
            ...
            {
                "Issue": "Dust Filter is due to be replaced or cleaned",
                "Recommendation": """Root Cause:
...
- Always back up BitLocker key to Microsoft account or external drive."""
    ...
    def get_all(self):
        """Return all issue–recommendation data."""
        return self.data

    def get_recommendation(self, issue_name):
        """Return recommendation text for a given issue (case-insensitive match)."""
        issue_name_lower = issue_name.lower()
        for item in self.data:
            if item["Issue"].lower() == issue_name_lower:
